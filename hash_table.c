...
static entry_t *find_previous_entry_for_key(entry_t *prev_entry, elem_t key, hash_function hash)
{
  if (hash == NULL)
  {
    ...
    { //  We can't go any futher down the list
      return prev_entry;
    }
    else if (abs(current_entry->key.int_value) >= abs(key.int_value))
    { //  We found the right place to insert a new entry
      return prev_entry;
    }
    else
    {
...
static void hash_table_insert(ioopm_hash_table_t *ht, entry_t *entry, elem_t key, elem_t value, bool key_is_int)
{
  ...
  if (key_is_int == true)
  {
    if (next != NULL)
    {
      if (abs(next->key.int_value) == key.int_value)
      { // Check if next has our key, if not create and entry
        next->value = value;
      }
      else
      {
  ...
  else
  {
    if (next != NULL)
    {
      if (strcmp(next->key.string_value, key.string_value) == 0)
      { // Check if next has our key, if not create and entry
        next->value = value;
      }
      else
...
static ioopm_option_t hash_table_lookup(entry_t *entry, elem_t key, bool key_is_int)
{
  ...
  if (key_is_int == true)
  {
    if (next != NULL)
    {
      if (next->key.int_value == key.int_value)
      { // Check if keys as string are equal
...
static ioopm_option_t hash_table_remove(ioopm_hash_table_t *ht, entry_t *entry, elem_t key)
{
  ...
  else
  {
    if (next != NULL)
    {
      if (ht->hash_fun(next->key) == ht->hash_fun(key))
      { // Keys as strings
        ioopm_option_t options = {.success = true, .value = next->value};
        entry->next = next->next;
        free(next);
...
bool ioopm_hash_table_all(ioopm_hash_table_t *ht, ioopm_predicate pred, void *arg)
{
  ...
  bool *extra = false;
  for (int i = 0; i < size; link = link->next, link_itr = link_itr->next, i++)
  { // Goes through the whole list
    if (!pred(link->element, link_itr->element, arg, extra))
    { // If pred does not satisfy for some key/value pair => return false
...
bool ioopm_hash_table_any(ioopm_hash_table_t *ht, ioopm_predicate pred, void *arg)
{
  ...
  { // Goes through the whole list
    if (pred(link->element, link_itr->element, arg, extra))
    { // If pred does satisfy for some key/value pair => return true
      ioopm_linked_list_destroy(linked_list);
      ioopm_linked_list_destroy(list_v);
      return true;
    }
...
bool ioopm_hash_table_has_key(ioopm_hash_table_t *ht, elem_t key)
{
  ...
  if (ioopm_hash_table_lookup(ht, key).success == true)
  {
    return true;
  }
  else
